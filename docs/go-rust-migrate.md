...
- **Story 1.1: Project Skeleton & Modules** [x]
...
1.  **Story 1.3:** Create `src/db.rs` and set up the `PgPool`.
2.  **Story 1.2:** Update `main.rs` to use the `Settings` struct from `config.rs`.
3.  **Story 2.1:** Define the `User` model to match the database schema.